...
async function resolveFunctionErrorMessage(error) {
  ...
  if (error instanceof FunctionsRelayError) {
    return 'Supabase could not relay the Gemini request to the Edge Function right now.';
  }
...
export async function requestListingCopilotSuggestions(input) {
  ...
  if (
    ...
    throw new Error(
      'Add a short prompt or a few draft details first so AI Listing Copilot has something to improve.'
    );
  }
